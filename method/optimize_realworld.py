...
# @Author  : YOUR-NAME
# @FileName: optimize_realworld.py
# @Software: PyCharm
...
def calculate_2norm_diag(feature_matrix, order=1, my_eps=1e-4):
    # my_eps = 1e-10
    # denominator = np.linalg.norm(feature_matrix, axis=0)
    # denominator = np.maximum(denominator, my_eps)

    # feature_matrix = np.maximum(feature_matrix, 1e-4)
    if order == 1:
        col_sum = 1 / np.maximum(np.linalg.norm(feature_matrix, axis=0), my_eps)
        lambda_mat = np.diag(col_sum)
        return lambda_mat
    else:
        col_sum = 1 / np.maximum(np.linalg.norm(feature_matrix, axis=0) ** order, my_eps)
        lambda_mat = np.diag(col_sum)
        return lambda_mat
...
def update_fl(wl_mat, bl_mat, fl_mat, laplacian_mat, f_mat, para_list, my_eps=1e-10):
    ...
    # numerators = 2 * bl_mat.T @ bl_mat @ fl_mat + 2 * fl_mat @ laplacian_mat + 4 * alpha * fl_mat @ fl_mat.T @ fl_mat
    ...
    # denominators = 2 * bl_mat.T @ wl_mat + 4 * alpha * fl_mat @ f_mat.T @ fl_mat
...
def compute_loss(w_list, b_list, f_list, laplacian_list, f_mat, z_mat, para_list):
    ...
    for wl_mat, bl_mat, fl_mat, laplacian_mat in zip(w_list, b_list, f_list, laplacian_list):
        ...
        '''
        diag_sim_mat = np.maximum(diag_sim_mat, 1e-10)
        k_mat = np.maximum(k_mat, 1e-10)

        log_value = diag_sim_mat / k_mat
        result = np.sum(-1 * np.log(log_value))
        '''

        result = np.sum(-1 * np.log(np.maximum(diag_sim_mat, 1e-4) / np.maximum(k_mat, 1e-4)))
        loss4 += beta * result
